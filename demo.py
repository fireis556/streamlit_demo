...
if total:
    ...
    with sub_5:
        ...
        if reset:
            st.session_state.count = 0
    ori_df = pd.DataFrame(update_df(remaining_usr_list,TAR_NUM,MIN_VAL,MAX_VAL, direction).iloc[[st.session_state.count]]).style.applymap(highlight_col, subset=pd.IndexSlice[:, [f'U{choose}']])
